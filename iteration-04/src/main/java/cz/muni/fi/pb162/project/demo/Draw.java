...
public class Draw extends JFrame {
    ...
    protected void paintRegularPolygon(Graphics g, RegularPolygon polygon) {
        ...
        for (int i = 0; i < polygon.getNumEdges(); i++) {
            xVertex[i] = (int) ((halfX + polygon.getCenter().getX()) - polygon.getRadius() * Math
                .cos(i * 2 * Math.PI / polygon.getNumEdges()));
            yVertex[i] = (int) ((halfY - polygon.getCenter().getY()) - polygon.getRadius() * Math
                .sin(i * 2 * Math.PI / polygon.getNumEdges()));
        }
