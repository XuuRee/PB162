...
public class ProjectTest {
    ...
    public void task01() {
        ...
        try {
            assertTrue("distance() nevraci -1 jako indikator chybneho argumentu",
                new Vertex2D(-1, 0).distance(null) == -1.0);
        } catch (Exception ex) {
    ...
    public void task02() {
        ...
        assertTrue("isEquilateral()",
            new Triangle(new Vertex2D(-1.2, 0), new Vertex2D(1.2, 0), new Vertex2D(0, 2.07846097)).isEquilateral());
        assertFalse("isEquilateral()",
            new Triangle(new Vertex2D(-1.2, 0), new Vertex2D(1.2, 0), new Vertex2D(0, 5.0)).isEquilateral());

        try {
            Triangle ntri = new Triangle(null, null, null);
            assertEquals("toString vraci spatny vystup pokud je nektery z vrcholu prazdny", "INVALID TRIANGLE",
                ntri.toString());
    ...
    public void task04() {
        ...
        assertTrue("Parametricky konstruktor: spatny stred",
            c2.getCenter().getX() == 1.0 && c2.getCenter().getY() == -1.0);
        assertTrue("Bezparametricky konstruktor: spatny polomer", c1.getRadius() == 1.0);
        assertTrue("Bezparametricky konstruktor: spatny stred",
            c1.getCenter().getX() == 0.0 && c1.getCenter().getY() == 0.0);
        //c2.setRadius(0.3);
